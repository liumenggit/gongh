...
util.request = async function (option) {
	...
	var md5 = require('md5.js');
    var app = getApp();
   var area = option.data.area ?  option.data.area : await app.getArea()
  
    option.data.area = Number(area)
...
util.getNewUrl = function (option) {
  var url = option,
    sessionid = wx.getStorageSync("userInfo").sessionid;
  if (-1 == url.indexOf("http://") && -1 == url.indexOf("https://") && (url = util.url(url)),
    getUrlParam(url, "state") || option.data && option.data.state || !sessionid || (url = url + "&state=we7sid-" + sessionid), !option.data || !option.data.m) {
    var nowPage = getCurrentPages();
    nowPage.length && (nowPage = nowPage[getCurrentPages().length - 1]) && nowPage.__route__ && (url = url + "&m=" + nowPage.__route__.split("/")[0]);
  }
  var sign = getSign(url, option.data);
  return sign && (url = url + "&sign=" + sign), url;
},
